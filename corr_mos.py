...
for key, values in mos_data.items():
    for v in values:
        if v[0] == 4:
            mos_4x.append(v[1])
            jnd_4x.append(v[2])
...
# 상관계수 계산
mos_corr_4x, _ = pearsonr(mos_4x, jnd_4x)
mos_corr_8x, _ = pearsonr(mos_8x, jnd_8x)
mos_corr_16x, _ = pearsonr(mos_16x, jnd_16x)

print(f"MOS vs JND correlation for 4x: {mos_corr_4x}")
print(f"MOS vs JND correlation for 8x: {mos_corr_8x}")
print(f"MOS vs JND correlation for 16x: {mos_corr_16x}")
